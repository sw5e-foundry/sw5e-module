...
function buildReloadNode(item, app) {
  ...
  const numInputListener = app.isEditable
    ? {
        change: (event) => {
          ...
          _children: [
            [
              "a",
              {
                ...
                _listeners: app.isEditable
                  ? {
                      click: (event) => {
                        item.system.configureAmmo();
                      },
                    }
                  : {},
      ...
      [
        "a",
        {
          ...
          "data-tooltip": game.i18n.localize(
            `SW5E.WeaponAmmo.Type.${reload.type.capitalize()}.Hint.${reload.disabled ? "NoAmmo" : reload.full ? "Full" : "Default"}`
          ),
          _children: [["text", reload.action]],
          _listeners: app.isEditable
            ? {
                click: (event) => {
                  item.system.reloadWeapon();
                },
              }
            : {},
...
function addAmmoHelpers() {
      ...
      return {
        ...
        baseUse:
          this.actionType === "save"
            ? Math.max(this.getProperty("burst"), this.getProperty("rapid"), 1)
            : 1,
  ...
  Object.defineProperty(dnd5e.dataModels.item.WeaponData.prototype, "ammo", {
    set: async function (newAmmoID) {
      ...
      if (oldAmmo && oldLoad) {
        ...
        switch (oldAmmo.system.type.subtype) {
          ...
          case "projectorTank":
            if (oldLoad === reload.max)
              ammoUpdates["system.quantity"] = (oldAmmo.system.quantity ?? 0) + 1;
            else {
    ...
    get: function () {
      return this.reload.target ? this.parent.actor?.items?.get(this.reload.target) : null;
    },
  });
  ...
  dnd5e.dataModels.item.WeaponData.prototype.canUseAmmo = function (
    { amount, error, warn } = { warn: true }
  ) {
    const reload = this.reload;
  ...
  dnd5e.dataModels.item.WeaponData.prototype.useAmmo = function (
    { amount, error, warn, update, force } = { error: true, update: true }
  ) {
    const reload = this.reload;
