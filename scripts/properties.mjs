export function patchProperties() {
	Hooks.on("renderItemSheet5e", (app, html, data) => {
		html.find(".weapon-properties").each(async (idx, el) => {
			...
			boolNode.setAttribute("class", "form-group stacked weapon-properties");
			for (const prop of boolProperties) {
				...
				inputNode.setAttribute("name", `system.properties.${prop}`);
			...
			numericNode.setAttribute("class", "form-group grid weapon-properties");
